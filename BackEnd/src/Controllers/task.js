...
const ViewTask = asyncHandler( async (req, res) => {
      ...
      try {
          ...
          if (task.ownerID.toString() !== UserID && task.assignedUserID.toString() !== UserID) {
              return res.status(403).json({ message: 'Unauthorized User' });
          }
          let owner = await User.findById(task.ownerID);
          let assignedUser = await User.findById(task.assignedUserID);
          
          let dependTask;
          if(task.dependencyOnAnotherTask){
            console.log("Depened task: " + task.dependencyOnAnotherTask)
            dependTask = await Task.findById(task.dependencyOnAnotherTask)
            console.log("Title: "+dependTask.title)  ;
        }else dependTask = {title: null};
          const taskReturn = {
            "_id": task._id,
            ...
            "dependencyOnAnotherTask": task.dependencyOnAnotherTask,
            "dependTaskTitle": task.dependencyOnAnotherTask ?  dependTask.title : null,
            "status": task.status,
    

          }
          
        //   task.assignedUser = await User.findById(task.assignedUserID);
        //   task.description = owner.userName;
  
          return res.status(200).json({ message: 'Task retrieved successfully', taskReturn });
      } catch (error) {
...
const AssignTask =  asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const { AssignedID, title, description, category, dueDate, status, dependencyOnAnotherTask } = req.query;
        console.log(req.query);
        let givenDueDate = new Date(dueDate);
      ...
      try {
        ...
        //   const tasks = await Task.find();
          ...
          for(let i = 0; i < tasks.length; i++) {
            ...
            let owner = await User.findById(task.ownerID);
            let assignedUser = await User.findById(task.assignedUserID);
            let dependTask = await Task.findById(task.dependencyOnAnotherTask);
            ...
            let taskReturn = {
              ...
              "category":task.category,
              "dependencyOnAnotherTask": task.dependencyOnAnotherTask,
              "dependTaskTitle": dependTask ? dependTask.title : "",
              "status": task.status,
      
  
            }
            TaskReturns.push(taskReturn);
  
          }
  
          return res.status(200).json({ message: 'Tasks retrieved successfully', TaskReturns });
      } catch (error) {
            console.log(error);
